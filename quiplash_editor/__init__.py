...
SAFETY_QUIPS_DEFAULT = ["jus de gouille", "le buszy", "jean-marc"]
    ...
    def to_version(self, target_version=None):
        ...
        if target_version is None:
            ...
            raise UnknownPromptVersionError(
                f"Cannot convert to unknown target_version {target_version}"
            )
...
class QuipPromptV1(QuipPromptBase):
    ...
    def from_jet_entry(cls, entry: dict):
        ...
        if ver == 1:
            return cls(id=int(entry["id"]), prompt=entry["prompt"], x=bool(entry["x"]))
        elif ver == 3:
            return cls(
                id=int(entry["id"]),
...
class QuiplashReader:
    ...
    def prompt_class(self):
        """the QuiplashPrompt class we target. Default to v1"""
        if self.target_version is None or self.target_version == 1:
            return QuipPromptV1
        elif self.target_version == 3:
            return QuipPromptV3
    @property
    ...
    def load_csv_row(self, row):
        ...
        self.quip_prompts.append(self.prompt_class.from_csv_entry(row))
